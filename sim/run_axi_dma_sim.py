...
BUILD_DIR = REPO_ROOT / "build" / "sim" / "axi_dma"
SUMMARY_PATH = BUILD_DIR / "sim_summary.json"

TEST_NAMES = (
    "axi_dma_identity",
    "axi_dma_mixed_signed",
    "axi_dma_ar_backpressure",
    "axi_dma_rvalid_delay",
    "axi_dma_aw_backpressure",
    "axi_dma_w_backpressure",
    "axi_dma_bvalid_delay",
    "axi_dma_rresp_error",
    "axi_dma_read_timeout",
    "axi_dma_bresp_error",
    "axi_dma_write_timeout",
    "axi_dma_irq_done",
)
...
def write_summary(status, tests_passed=0, passed_names=None, notes=None):
    ...
    summary = {
        "status": status,
        "top": "tinynpu_axi_dma_wrapper",
        "testbench": "tb_tinynpu_axi_dma_wrapper",
        "mac_variant": "row4",
        "axi_lite_control": True,
        "axi_read_master": True,
        "axi_write_master": True,
        "bursts_supported": False,
        "tests_passed": tests_passed,
        "test_names": names,
        "axi_write_backpressure_verified": (
            "axi_dma_aw_backpressure" in names
            and "axi_dma_w_backpressure" in names
            and "axi_dma_bvalid_delay" in names
        ),
        "axi_read_backpressure_verified": (
            "axi_dma_ar_backpressure" in names
            and "axi_dma_rvalid_delay" in names
        ),
        "axi_rresp_error_verified": "axi_dma_rresp_error" in names,
        "axi_bresp_error_verified": "axi_dma_bresp_error" in names,
        "read_timeout_verified": "axi_dma_read_timeout" in names,
        "write_timeout_verified": "axi_dma_write_timeout" in names,
        "irq_verified": "axi_dma_irq_done" in names,
        "notes": notes
        or "AXI4-Lite control plus single-beat AXI4 read master for A/B and AXI4 write master for C; no bursts",
...
def main():
    ...
    BUILD_DIR.mkdir(parents=True, exist_ok=True)
    sim_out = BUILD_DIR / "tinynpu_axi_dma_wrapper.vvp"
    sources = [
        "rtl/tinynpu_pkg.sv",
    ...
    if status != "passed":
        print(
            f"ERROR: AXI DMA simulation status={status}, "
