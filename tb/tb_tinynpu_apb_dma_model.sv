...
module tb_tinynpu_apb_dma_model;
  ...
  task automatic dma_run_matmul(input int a_base, input int b_base, input int c_base);
    begin
  ...
  task automatic run_dma_case(input string test_name, input int a_base, input int b_base, input int c_base);
    int local_failures;
    begin
      dma_run_matmul(a_base, b_base, c_base);
  ...
  task automatic run_dma_back_to_back;
    int local_failures0;
    int local_failures1;
    begin
      ...
      dma_run_matmul(EXT_A0_BASE, EXT_B0_BASE, EXT_C0_BASE);
      dma_run_matmul(EXT_A1_BASE, EXT_B1_BASE, EXT_C1_BASE);

      check_c_region("dma_back_to_back_first", EXT_A0_BASE, EXT_B0_BASE, EXT_C0_BASE, local_failures0);
      check_c_region("dma_back_to_back_second", EXT_A1_BASE, EXT_B1_BASE, EXT_C1_BASE, local_failures1);
      if ((local_failures0 == 0) && (local_failures1 == 0)) begin
        tests_passed = tests_passed + 1;
        $display("PASS dma_back_to_back");
      end
    end
  endtask

  task automatic run_dma_external_memory_unchanged;
    int i;
    int local_failures;
    begin
      local_failures = 0;
      ...
      for (i = 0; i < 8; i = i + 1) begin
        if (ext_mem[EXT_SENTINEL_BASE + i] !== (32'sh1357_0000 + i)) begin
          $display("FAIL dma_external_memory_unchanged: ext_mem[%0d] changed to 0x%08x",
      ...
      if (local_failures == 0) begin
        tests_passed = tests_passed + 1;
        $display("PASS dma_external_memory_unchanged");
      end
    end
  endtask
  ...
  initial begin
    ...
    init_identity_case(EXT_A0_BASE, EXT_B0_BASE);
    run_dma_case("dma_identity", EXT_A0_BASE, EXT_B0_BASE, EXT_C0_BASE);

    init_mixed_case(EXT_A0_BASE, EXT_B0_BASE);
    run_dma_case("dma_mixed_signed", EXT_A0_BASE, EXT_B0_BASE, EXT_C0_BASE);

    clear_ext_mem();
    run_dma_back_to_back();

    clear_ext_mem();
    run_dma_external_memory_unchanged();

    $display("DMA-model tests passed: %0d", tests_passed);
    if (failures == 0) begin
      $display("tinyNPU APB DMA-model SIM PASS");
      $finish;
    end else begin
      $display("tinyNPU APB DMA-model SIM FAIL failures=%0d", failures);
      $fatal(1);
    end
  end
endmodule
