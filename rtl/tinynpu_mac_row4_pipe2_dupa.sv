...
module tinynpu_mac_row4_pipe2_dupa (
  input  logic                  clk,
  input  logic                  rst_n,
  input  logic                  start,
  ...
  logic [1:0] row_q;
  logic [1:0] k_q;
  logic signed [`DATA_W-1:0] sel_a0_q;
  logic signed [`DATA_W-1:0] sel_a1_q;
  logic signed [`DATA_W-1:0] sel_a2_q;
  logic signed [`DATA_W-1:0] sel_a3_q;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state_q)
        ...
        S_SEL_ROW: begin
          sel_a0_q <= get_a(row_q, k_q);
          sel_a1_q <= get_a(row_q, k_q);
          sel_a2_q <= get_a(row_q, k_q);
          sel_a3_q <= get_a(row_q, k_q);
        ...
        S_MUL_ROW: begin
          prod0_q <= product_at(sel_a0_q, sel_b0_q);
          prod1_q <= product_at(sel_a1_q, sel_b1_q);
          prod2_q <= product_at(sel_a2_q, sel_b2_q);
          prod3_q <= product_at(sel_a3_q, sel_b3_q);
          state_q <= S_ACC_ROW;
        end
